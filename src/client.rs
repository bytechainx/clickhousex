...
/// 内部 URL 构造。
fn build_url(config: &ClickHouseConfig, params: &[(&str, &str)]) -> ClickHouseResult<url::Url> {
...
/// 是否需要发送 Basic 认证头。
fn needs_basic_auth(config: &ClickHouseConfig) -> bool {
    config.user != crate::config::DEFAULT_USER || !config.password.is_empty()
}

/// 校验 SQL 标识符（表名等），避免拼接出非法或注入型语句。
fn validate_ident(name: &str) -> ClickHouseResult<()> {
...
/// 校验查询参数名。
fn validate_params(params: &[(&str, &str)]) -> ClickHouseResult<()> {
...
fn encode_rows(rows: &[Value]) -> ClickHouseResult<Vec<String>> {
...
fn build_http_client(config: &ClickHouseConfig) -> ClickHouseResult<reqwest::Client> {
...
fn map_transport_error(error: &reqwest::Error) -> ClickHouseError {
    if error.is_timeout() {
...
fn map_http_error(status: StatusCode, body: &[u8]) -> ClickHouseError {
...
async fn read_error_prefix(mut response: reqwest::Response) -> ClickHouseResult<Vec<u8>> {
